...
**ROS (Robot Operating System)** adalah framework middleware open-source yang dirancang khusus untuk pengembangan robot. Meskipun namanya "Operating System", ROS bukanlah sistem operasi seperti Linux atau Windows, melainkan **framework middleware** yang menyediakan:
...
Dengan ROS, developer tidak perlu membangun sistem komunikasi dari nol. ROS sudah menyediakan mekanisme untuk menghubungkan sensor, aktuator, algoritma, dan komponen lainnya secara efisien.

## Sejarah Singkat ROS

ROS awalnya dikembangkan oleh **Willow Garage** pada tahun 2007 dan kemudian dikelola oleh **Open Robotics** (sekarang bagian dari Intrinsic/Alphabet). Terdapat dua generasi utama:

### ROS 1 (2007–2025)

- Dimulai dengan distribusi pertama **ROS Box Turtle** (2010).
- Distribusi terkenal: **Kinetic** (Ubuntu 16.04), **Melodic** (Ubuntu 18.04), **Noetic** (Ubuntu 20.04).
- **ROS 1 Noetic Ninjemys** (Mei 2020) adalah distribusi **terakhir** ROS 1, dengan EOL Mei 2025.
- Memerlukan `roscore` (master node) sebagai pusat komunikasi.
- Hanya didukung resmi di Linux (Ubuntu).
- Tidak mendukung real-time, multi-robot, atau keamanan secara native.

### ROS 2 (2017–sekarang)

ROS 2 ditulis ulang dari nol untuk mengatasi keterbatasan ROS 1:

- **Decentralized** — tidak butuh `roscore`, menggunakan DDS (Data Distribution Service).
- **Multi-platform** — mendukung Linux, Windows, macOS.
- **Real-time** — mendukung eksekusi real-time untuk kontrol robot.
- **Security** — enkripsi dan autentikasi bawaan via DDS Security.
- **Multi-robot** — dukungan native untuk banyak robot dalam satu jaringan.

Distribusi ROS 2 yang penting:

| Distribusi | Rilis | Ubuntu | Status |
|------------|-------|--------|--------|
| **Foxy Fitzroy** | Jun 2020 | 20.04 | EOL Mei 2023 |
| **Humble Hawksbill** | Mei 2022 | 22.04 | LTS, EOL Mei 2027 |
| **Iron Irwini** | Mei 2023 | 22.04 | EOL Nov 2024 |
| **Jazzy Jalisco** | Mei 2024 | 24.04 | LTS, EOL Mei 2029 |

> **Modul ini menggunakan ROS 2 Jazzy** sebagai distro utama untuk praktik. Konsep-konsep yang dibahas berlaku untuk semua distribusi ROS 2, dan sebagian besar juga relevan untuk ROS 1.

### ROS 1 vs ROS 2 — Ringkasan

| Aspek | ROS 1 | ROS 2 |
|-------|-------|-------|
| Arsitektur | Centralized (`roscore`) | Decentralized (DDS) |
| Real-time | Tidak | Ya |
| Platform | Linux only | Linux, Windows, macOS |
| Security | Tidak ada | DDS Security |
| Multi-robot | Sulit | Native |
| Build system | `catkin` | `colcon` + `ament` |
| Client library | `rospy` / `roscpp` | `rclpy` / `rclcpp` |
...
<img alt="RViz2 - 3D Visualization" src="img/rviz.png">
...
<img alt="Gazebo - Robot Simulator" src="img/gazebo.png">
...
- **ROS 2 Docs (Jazzy)**: https://docs.ros.org/en/jazzy/
- **ROS 2 Docs (Humble)**: https://docs.ros.org/en/humble/
- **ROS 1 Wiki (Noetic)**: http://wiki.ros.org/noetic
...
- **ROS** adalah framework middleware untuk robotika, terdiri dari **ROS 1** (legacy) dan **ROS 2** (aktif).
- ROS 1 Noetic adalah distribusi terakhir ROS 1; migrasi ke ROS 2 sangat direkomendasikan.
- ROS 2 menggunakan **DDS** untuk komunikasi yang decentralized, real-time, dan secure.
- Distribusi LTS ROS 2 saat ini: **Humble** (Ubuntu 22.04) dan **Jazzy** (Ubuntu 24.04).
- Konsep dasar: Nodes, Topics, Services, Actions, Parameters.
- Mendukung multiple platform dan bahasa pemrograman (Python, C++).
- Cocok untuk pengembangan robot profesional, penelitian, maupun pembelajaran.

## Next Steps

Lanjut ke [Instalasi ROS 2](02_instalasi.md) untuk memulai setup environment Anda!
